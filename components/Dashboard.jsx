...
export default function Dashboard({
  ...
  // (매시 00분·30분), 주간/일간/분야별 탭은 이제 전부 cron-job.org가
  // 매일 07:00 KST에 각 workflow_dispatch(collect.yml/collect-daily.yml)를
  // 호출하는 동일한 방식이라 문구도 다시 하나로 합쳤습니다(2026-08-26 -
  // collect-daily.yml이 GitHub 자체 schedule을 쓰다가 첫 회차가 약
  // 20시간 지연 발화되면서 등락 계산 기준 시점이 어긋난 문제가 있어,
  // collect.yml과 같은 07:00 cron-job.org 방식으로 통일함). 이 문구는
  // 화면 표시용 안내일 뿐이라 실제 수집 로직 자체는 건드리지 않았습니다.
