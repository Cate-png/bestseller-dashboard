...
export default function Dashboard({
  ...
  // 헤더 "최종 업데이트" 옆 ⓘ 툴팁 문구. 실시간 탭은 수집 주기가 다르고
  // (매시 00분·30분), 일간 탭은 collect-daily.yml이 매일 06:00 KST에
  // 그대로 도는 반면, 주간/분야별 탭은 collect.yml을 트리거하는 외부
  // cron-job.org 스케줄을 06:00 -> 07:00 KST로 변경했으므로(2026-08-26,
  // 사용자 요청 - 알라딘 "주간" 순위가 실제 사이트와 하루 종일 계속
  // 바뀌는 문제와는 별개로, 최소한 갱신 시각 표기를 실제 수집 시각과
  // 맞추기 위함) 문구를 분리했습니다. 이 문구는 화면 표시용 안내일 뿐이라
  // 실제 수집 로직 자체는 건드리지 않았습니다.
