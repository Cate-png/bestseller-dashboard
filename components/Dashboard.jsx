...
export default function Dashboard({
  ...
  // 헤더 "최종 업데이트" 옆 ⓘ 툴팁 문구. 기본 문구는 모든 탭이 공유하고
  // (사용자 요청으로 "수집 시각: ..." 줄 삭제, 2026-08-26 - 탭별 수집
  // 주기 안내가 필요 없어지면서 실시간/그 외 탭 문구를 굳이 나눌 이유도
  // 없어져 하나로 합쳤습니다), 주간 탭(종합 주간 + 분야별)에서만 한 문장을
  // 덧붙입니다. 2026-08-30 사용자 실측: 교보문고 "소설" 분야에서 오전
  // 7시 수집분(8위·10위 상승)과 같은 날 오후 2시 3분 교보 실제 페이지
  // (3위·5위 상승)가 서로 달랐고, 등락 기준값이 8→8로 정확히 맞물려
  // 교보 자체가 "주간" 랭킹을 하루 중 다시 계산한다는 게 확인됨(3분
  // 간격 재수집으로는 변화 없어 분 단위는 아님). 일간/실시간 탭은 각자
  // 스스로의 갱신 주기 문제라 이 문장을 붙이지 않습니다. 화면 표시용
  // 안내일 뿐이라 실제 수집 로직 자체는 건드리지 않았습니다.
  const updateInfoText =
    "각 서점의 베스트셀러 데이터는 특정 시점에 수집한 결과입니다. 서점별 집계·업데이트 시점에 따라 실제 서점 페이지와 차이가 있을 수 있습니다." +
    (!isRealtime && !isDaily
      ? " 주간 베스트셀러는 서점 자체에서도 하루 중 다시 계산되는 경우가 있어, 저희 수집 시점 이후 서점 실제 페이지의 순위·등락과 다를 수 있습니다."
      : "");
  ...
  // 분야 목록에서 연 책은 "소설" 분야 TOP100 안에서의 순위 이력을 보여주고,
